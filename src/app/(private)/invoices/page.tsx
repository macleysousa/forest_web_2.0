...
import { MdArrowDownward, MdArrowDropDown, MdArrowUpward, MdCalendarToday } from 'react-icons/md';
...
const formatPeriod = (period: string) => period.replace(/(\d{2}-\d{2})-\d{4}/g, '$1').replace(/-/g, '/');
const formatCNPJ = (cnpj: string) => cnpj.replace(/^(\d{3})(\d{3})(\d{3})(\d{4})(\d{2})$/, '$1.$2.$3/$4-$5');
...
export default function InvoicesPage() {
  ...
  useEffect(() => {
    getNFEs({ order: state.order, order_type: state.orderType, period: state.period, page: state.page, has_filters: '1' })
      .then((result) => {
  ...
  const handleOrderChange = (order: keyof typeof Order) => () => {
    ...
    setState({ ...state, order, page: '1', nfes: [], totalQuantity: 0, totalQuantityMix: 0, valueTotal: 0, cobertura: 0 });
  };
    ...
    <PrivateLayout>
      <Box padding="2rem">
        <Flex alignItems="center">
          ...
          <Flex marginLeft="auto" gap="1rem">
            <Box position="relative">
              ...
              >
                <Text as="span" color="#898989" display="inline-block">
                  Período
                </Text>
              </Button>
              <RangeDatepicker
                selectedDates={state.period.split(' a ').map((value) => new Date(`${value.split('-').reverse().join('-')}T00:00:00.000-03:00`))}
                onDateChange={(date) => console.log(date)}
                configs={{ monthNames, dayNames, firstDayOfWeek: 0 }}
                propsConfigs={{ inputProps: { position: 'absolute', left: '0', w: '100%', h: '2.5rem', size: 'sm', opacity: '0' } }}
              />
            </Box>
            <Menu>
              <MenuButton
            ...
            <Button size="sm" h="2.5rem" color="#1E93FF" bg="#FFFFFF" border="1px solid #1E93FF" isDisabled>
              Exportar
            </Button>
          </Flex>
        </Flex>
        <Flex marginTop="1.5rem" maxWidth="80rem" justifyContent="space-between" marginLeft="auto" marginRight="auto">
          {
            [
              { label: 'Período', value: (<>de {formatPeriod(state.period).split(' a ')[0]}<br />a {formatPeriod(state.period).split(' a ')[1]}</>), size: 'lg' },
              { label: 'Volume', value: state.totalQuantity.toLocaleString('pt-BR'), size: '2xl' },
              { label: 'Volume Mix', value: state.totalQuantityMix.toLocaleString('pt-BR'), size: '2xl' },
              { label: 'Cobertura', value: state.cobertura.toLocaleString('pt-BR'), size: '4xl' },
              { label: 'Produtos', value: '00', size: '4xl' },
              { label: 'Valor Total', value: state.valueTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }), size: 'sm' },
                ...
                state.nfes.map((nfe, i) => (
                  ...
                  >
                    <Td padding="1.25rem 0.75rem">
                      <Box display="inline-flex" flexDirection="column" gap="0.5rem" alignItems="center" w="100%">
                        <Text as="span" fontSize="xs" textDecoration="underline">
                          {nfe.customer_emitter_social_name}
                        </Text>
                        <Text as="span" fontSize="xs" background="#E9F1F2" color="#70B6C1" textAlign="center" w="100%">
                    ...
                    <Td padding="1.25rem 0.75rem" textAlign="center">
                        <Text as="span" fontSize="xs">
                          {(nfe.date.split(' ')[0] ?? '').split('-').reverse().join('/')}
                        </Text>
                        <br />
                        <Text as="span" fontSize="xs">
                          {(nfe.date.split(' ')[1] ?? '').replace(/^(\d{2}:\d{2}):\d{2}$/, '$1')}
                        </Text>
                    </Td>
                    <Td padding="1.25rem 0.75rem">
                      <Box display="inline-flex" flexDirection="column" gap="0.5rem" alignItems="center" w="100%">
